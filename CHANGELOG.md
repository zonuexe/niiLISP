...
- **A library target** ([ADR-0039](docs/adr/0039-embedding-library-target.md)): niiLISP can now be embedded in a Rust program as an in-process interpreter — `use niilisp::{Interp, Value, Signal}`, then `Interp::new()` and `eval_string(&[u8]) -> Result<Value, Signal>`. `Value` and `Signal` gained `Debug` impls. See the README "Embedding" section and [`examples/embed.rs`](examples/embed.rs). The `niilisp` CLI is now a thin client of the same library. Caveats: `Interp` is single-threaded (`!Send`/`!Sync`), and the default features `fork`/network/FFI the host (use `default-features = false` for a sandbox); the `0.x` API is unstable. (Embedding-safe `(exit)`, host builtins, and a step limit followed in ADR-0040 above.)
