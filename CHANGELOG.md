...
- **Cilk API / fork multitasking** (ADR-0032), behind a default-on `mt` Cargo feature, Unix-only (real `fork()` of the interpreter via `libc`): `(spawn 'sym expr)` forks a child to evaluate `expr` in parallel, `(sync [timeout [inlet]])` waits and binds each child's result to its symbol (calling an optional inlet per finish), `(abort [pid])` cancels children; `(share)` / `(share adr val)` / `(share adr)` exchange a value through an `mmap`ed shared page; raw `(fork expr)`, `(pipe)` (returns two file handles), and `(wait-pid pid)`. Cross-process values transfer as their re-readable `repr`. Passes the vendored `qa-cilk`, `qa-share`, and `qa-pipefork`. Adds `write-line` (always-on) and the `do-until`/`do-while` post-test loops.
